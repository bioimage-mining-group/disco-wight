...
pixelSizeZ_ = 1.6; % Anisotropy in Z
timeInterval_ = 1;


%all_SRs = [2 4 6 8 10 12 14 16];
all_SRs = 1:1:16;
all_names = ["cp_cyto2" "sd" "cp_trained" "sd_trained" "gt"];
...
for name = all_names
    ...
    for SR = all_SRs
        ...
        % f'{path_to_root}/dataset_organoid/utrack/{dataset_index}/{model}/utrack_inputs'
        path_to_inputs = strcat(path_to_root,'/dataset_organoid/utrack/',num2str(dataset_index),'/',name,'/utrack_inputs');
        path_to_results = strcat(path_to_root,'/dataset_organoid/utrack/',num2str(dataset_index),'/',name);
    ...
    %% Detection parameterization and processing
        ...
        for fIdx=1:nFrames
            ...
            obj = Tiff(strcat(fullpath1, '/',directory_infos(fIdx).name));
            i = 1;
            vol = zeros(91,91, 23);
            while 1
                subimage = obj.read();
