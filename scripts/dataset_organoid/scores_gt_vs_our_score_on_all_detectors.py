...
"""
This script is used to generate the following figures for the organoid dataset:
...
scale4d = (1,1,0.621,0.621)
scale3d = np.array(scale4d[1:])

path_to_root = ...

path_to_labels = f'{path_to_root}/data'
path_to_tracks = f'{path_to_root}/utrack'
path_to_save = f'{path_to_root}/analysis_results'
path_to_figures = f'{path_to_root}/figures'



all_SRs = np.arange(1,17);
all_F1Rs = np.arange(0.5,5.5,0.5)
...
label_names = [
    ...
    'labels_predicted_best_cp',
	'labels_predicted_sd_pretrained',
    'labels_predicted_best_sd',
]
...
if not plot:
    ...
    for ind_name, (name, label_name) in tqdm(enumerate(zip(folder_names[1:], label_names[1:]))):
        ...
        labels_gt = tifffile.imread(f'{path_to_labels}/labels_gt.tif')
        coords_gt = [np.array([prop.centroid for prop in regionprops(img)]) for img in labels_gt]

        labels_pred = tifffile.imread(f'{path_to_labels}/{label_name}.tif')
        ...
        F1c_to_save.append(F1s_of_model)
        F1c_std_to_save.append(F1s_std_of_model)
        F1iou_to_save.append(F1s_iou_of_model)
        F1iou_std_to_save.append(F1s_iou_std_of_model)

    np.savetxt(f'{path_to_save}/F1c.txt', F1c_to_save)
    np.savetxt(f'{path_to_save}/F1c_std.txt', F1c_std_to_save)
    np.savetxt(f'{path_to_save}/F1iou.txt', F1iou_to_save)
    np.savetxt(f'{path_to_save}/F1iou_std.txt', F1iou_std_to_save)
    ...
    for ind_name, (name, label_name) in tqdm(enumerate(zip(folder_names, label_names))):
        ...
        for ind_SR, SR in enumerate(all_SRs):
            ...
            path_to_trackability = f'{path_to_tracks}/{name}/{ind_SR}'
        ...
        Tys_to_save.append(Ty_model)
        Tys_std_to_save.append(Ty_std_model)
    
    np.savetxt(f'{path_to_save}/Tys.txt', Tys_to_save)
    np.savetxt(f'{path_to_save}/Tys_std.txt', Tys_std_to_save)
    ...
    for i, (Ty, Tystd, color) in enumerate(zip(Tys_to_save[:,ind_SR], Tys_std_to_save[:,ind_SR25], colors)):
        ax.bar(
            x=i, 
            height=Ty, 
            yerr=Tystd, 
